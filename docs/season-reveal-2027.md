...
- Poster: the requested **0:05** title-card still (white B logo, “Bardownski Hockey Club” and “NHL 27”), captured from the published film at native 1920×1080 and encoded as lossless WebP at `public/images/announcements/bardownski-2027-00-05.webp`. The new URL avoids stale optimized-image caches. The previous three-uniform poster is removed; `uniforms.png` must not be published or referenced by site code.
...
- On the homepage, the reveal appears **only as the featured card in Recent news**, using the requested 0:05 thumbnail. There is no standalone reveal block above the header or home introduction. The photo-backed **“Home ice. The next shift.”** hero remains first. Matches retains its original bench-photo header; the rejected generated rink graphic is removed.
- The featured news card opens the full article and its video player, rather than a homepage dialog. This is a real article link and works without JavaScript.
...
`SITE_URL` can target an optimized local build or the deployed site. The reveal browser check covers 1440/768/390/320px, News-only featured placement, monochrome poster, deferred video, article metadata/play/seek and captions, transcript, load-error recovery, roster leadership, no-JS article links/player and axe checks. Automated checks do not replace a complete manual accessibility audit.
...
- Homepage Recent matches, Player of the Week and MVP all use the supplied 2026–2027 season state. Recent matches links to the current-season match pages; unavailable data never falls back to frozen games or weekly winners. Previous award winners and the six Past seasons entries remain explicitly historical. The public connected-tracking banner is removed.
