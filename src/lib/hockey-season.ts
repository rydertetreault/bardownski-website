...
export function hockeyStateFromTracker(result: import("./hockey-tracker").TrackerResult): HockeySeasonState {
  ...
  const base = {season: HOCKEY_SEASON as typeof HOCKEY_SEASON, syncedAt: snapshot?.syncedAt ?? null, coverage:{storedMatches:result.matches.length,totalGames:snapshot?.data.clubStats.totalGames ?? null}, ...(result.error ? {error:result.error} : {})};
